...
TCPReceiver::TCPReceiver(int codingLen) :expectSequenceNumberRcvd(0), mask((1 << codingLen) - 1)
{
...
void TCPReceiver::receive(const Packet& packet)
{
	...
	pUtils->printPacket("TCP Receiver正确收到发送方的报文", packet);
	...
	lastAckPkt.checksum = pUtils->calculateCheckSum(lastAckPkt);
	pUtils->printPacket("TCP Receiver发送确认报文", lastAckPkt);
